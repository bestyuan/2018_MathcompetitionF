...
#判断换乘是否失败 时间不够则失败
def Judge(list_a):
    Interval = list_a["出发时间"]-list_a["到达时间"]
    a_time = Interval.total_seconds()/60
    #乘客花费时间
    if dict_time[list_a["到达类型"]+list_a["到达航班登机口"][0]+list_a["出发类型"]+list_a["出发航班登机口"][0]][0]<=a_time:
        list_a["换乘成功"]="yes"
    else:
        list_a["换乘成功"]="no"
    return dict_time[list_a["到达类型"]+list_a["到达航班登机口"][0]+list_a["出发类型"]+list_a["出发航班登机口"][0]][0]
...
if __name__ == "__main__":
    ...
    for i in TicketsSheet:
        ...
        # 有temp_position
        if i["到达航班登机口"] != "0" and i["出发航班登机口"] != "0"and i["到达航班登机口"]!="temp_position"and i["出发航班登机口"]!="temp_position":
            Judge(i)
...
GetFinal_result(Sorted_PucksSheet)
    ...
    #     out_airport = i["出发航班"]
    #     out_time = i["出发时间"]
    #     for j in TicketsSheet:
    #         if j['到达航班'] == arr_airport and j['到达时间'].strftime("%d-%b-%Y") == i["到达时间"].strftime("%d-%b-%Y"):
    #             j['到达时间'] = arr_time
    #         if j['出发航班'] == out_airport and j['出发时间'].strftime("%d-%b-%Y") == i["出发时间"].strftime("%d-%b-%Y"):
    #             j['出发时间'] = out_time
    # test1 = pd.DataFrame()
    # TicketsSheet = sorted(TicketsSheet, key=lambda puck: puck["旅客记录号"])
    # for i in TicketsSheet:
    #     i['diff_time'] = i['出发时间'] - i['到达时间']
    # test = pd.DataFrame(data=TicketsSheet)
    # test1["旅客记录号"] = test["旅客记录号"]
    # test1["乘客数"] = test["乘客数"]
    # test1['到达航班'] = test['到达航班']
    # test1['到达时间'] = test['到达时间']
    # test1['出发航班'] = test['出发航班']
    # test1['出发时间'] = test['出发时间']
    # test1['停留时间'] = test['diff_time']
    # test1.to_csv("add_time in Tickets.csv", encoding="gb2312", index=None)

    # TicketsSheet
    ##test count all located in airport and none in airport
    # list_none =[]
    # list_process=[]
    # for i in Sorted_PucksSheet:
    #     if i["label"]=="temp_position":
    #         list_none.append(i)
    #     else:
    #         list_process.append(i)
    # list_none=sorted(list_none, key=lambda puck: puck["diff_time"])
    # list_process =sorted(list_process, key=lambda puck: puck["diff_time"])
    # a=len(list_process)
    # for i in range(a-1,0):
    #     flag=0
    #     for j in  list_none:
    #         if  j["到达时间"]>=list_process[i]["到达时间"] and j["出发时间"]<=list_process[i]["出发时间"] and\
    #             j["到达类型"]==list_process[i]["到达类型"] and j["出发类型"]==list_process[i]["出发类型"] and\
    #             j["机体类别"]==list_process[i]["机体类别"]:
    #             temp_list=list_process[i]
    #             j["label"] = list_process[i]["label"]
    #             list_process[i]["label"]="temp_position"
    #             list_process.remove(list_process[i])
    #             list_process.append(j)
    #             list_none.remove(j)
    #             list_none.append(list_process[i])
    #             flag=1
    #             break
    #     if(flag==1):
    #         flag=0
    #         for j in list_none:
    #             if j["到达时间"]-temp_list["出发时间"]>2700 and j["出发时间"]<temp_list["出发时间"] and \
    #                j["到达类型"] == temp_list[i]["到达类型"] and j["出发类型"] == temp_list[i]["出发类型"] and \
    #                j["机体类别"] == temp_list[i]["机体类别"]:
    #                 j["label"]=temp_list["lable"]
    #                 list_process.append(j)
    ...
    # get no.1 problem
    # PucksSheet = sorted(Sorted_PucksSheet, key=lambda puck: puck["飞机转场记录号"])
    # on_date = []
    # on_time =[]
    # out_date=[]
    # out_time=[]
    ...
    #     out_time.append(i["出发时间"].strftime("%H:%M"))
    # test1 = pd.DataFrame()
    # test = pd.DataFrame(data=PucksSheet)
    # test1["飞机转场记录号"]=test["飞机转场记录号"]
    # test1["到达日期"]=on_date
    # test1["到达时刻"]=on_time
    # test1["到达航班"]=test["到达航班"]
    ...
    # test = pd.DataFrame(data=GatesSheet)
    # test1["登机口"] = test["登机口"]
    # test1["航班数量"]=test["航班数量"]
    # test1["宽体机"] = test["宽体机"]
    # test1["窄体机"]=test["窄体机"]
    # test1.to_csv("_he1.csv",encoding="gb2312",index=None)
    # get data end
